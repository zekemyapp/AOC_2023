...
int main (void)
{
	...
	for (string line : input) {
		...
		for(auto it=sequence.rbegin(); it != std::prev(sequence.rend()); it++) {
			...
			auto current_seq = *it;
			int64_t last_val_seq = current_seq.at(current_seq.size()-1);
			int64_t last_val_next_seq = next_seq.at(next_seq.size()-1);

			int64_t new_val = last_val_next_seq + last_val_seq;
			(*std::next(it, 1)).push_back(new_val);
		}

		subseq = sequence.at(0);

		int64_t last_value_in_seq = subseq.at(subseq.size()-1);
		cout << "last value in seq: " << last_value_in_seq << endl;
		output += last_value_in_seq;
	}
